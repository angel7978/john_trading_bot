...
class OrderBook:
    ...
    def generate_chart_data(self, symbol, timeframe='30m', limit=125, includeLatest=False):
        ...
        df['bb_bb_lp'] = (df['bb_bbh'] - df['bb_bbl']) / df['bb_bbm']

        indicator_bb_p = BollingerBands(close=df["bb_bb_lp"], window=100, window_dev=1)
        df['bb_bbh_p'] = indicator_bb_p.bollinger_hband()
        df['bb_bbm_p'] = indicator_bb_p.bollinger_mavg()
